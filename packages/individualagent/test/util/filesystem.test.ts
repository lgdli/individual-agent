...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "individualagent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "individualagent.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "individualagent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "individualagent.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["individualagent.json", "individualagent.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "individualagent.json"),
        path.join(tmp.path, "individualagent.jsonc"),
        path.join(project, "individualagent.json"),
        path.join(project, "individualagent.jsonc"),
      ])
    })
  })
