...
Replace runtime Bun shell template-tag usage in `packages/individualagent/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/individualagent/src` is removed except:
