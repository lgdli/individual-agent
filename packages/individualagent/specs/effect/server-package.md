...
Practical reference for extracting a future `packages/server` from the current `packages/individualagent` monolith while `packages/core` is still being migrated to Effect.
...
Everything still lives in `packages/individualagent`.
...
- the current server host still lives in `packages/individualagent`
...
- if `packages/server` imports services from `packages/individualagent` while `packages/individualagent` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/individualagent` as the temporary Hono host
...
- `packages/server` must not import from `packages/individualagent`

Allowed in phase 1:

- `packages/individualagent` imports `packages/server`
...
- `packages/individualagent` shrinks or disappears as package responsibilities are fully split
...
- `packages/individualagent` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/individualagent`
...
Keep in `packages/individualagent` for now:
...
- host mounting from `packages/individualagent`
...
- no imports from `packages/individualagent` inside `packages/server`
...
- `packages/server` must still not import from `packages/individualagent`
...
### PR 4. Mount `packages/server` question from `packages/individualagent`

Scope:

- replace local experimental question route wiring in `packages/individualagent`
...
- mount it in parallel from `packages/individualagent`
...
- move adapter selection and server startup out of `packages/individualagent`

Rules:

- do not start this while `packages/server` still depends on `packages/individualagent`
...
- host logic no longer lives in `packages/individualagent`
...
3. the route is mounted from the current host in `packages/individualagent`
...
- `packages/server` imports services or runtime from `packages/individualagent`
- `packages/individualagent` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/individualagent` imports
...
- [ ] mount `question` from `packages/individualagent`
...
2. keep `packages/individualagent` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/individualagent`, stop and restructure the boundary first.
