...
import { InstallationVersion } from "@individualagent/core/installation/version"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in individualagent.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  ...
  const candidates = [path.join(baseDir, "individualagent.json"), path.join(baseDir, "individualagent.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".opencode", "individualagent.json"), path.join(baseDir, ".opencode", "individualagent.jsonc"))
  }
  ...
  // Default to individualagent.json if none exist
  return candidates[0]
}
